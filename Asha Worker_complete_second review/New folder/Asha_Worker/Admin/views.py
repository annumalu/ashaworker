...
def AdminHome(request):
    if 'disable' in request.POST:
        ...
        exclude = Org_Profile.objects.filter(user__is_active=1).exclude(Auth_Id=val)

        return render(request, 'Admin/index.html', {'update': True, 'user': user, 'prof': prof, 'val': val,
                                                    'exclude': exclude})
        ...
        asha = Asha_Profile.objects.filter(Owner=basic.Auth_Id)
