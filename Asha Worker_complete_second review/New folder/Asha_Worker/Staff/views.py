...
from .models import Asha_Profile,Asha_Reports,Org_Asha_Notifications,Org_Asha_Forms,Asha_Insurance,Account
...
def StaffHome(request):
    if 'disable' in request.POST:
        ...
        exclude = Asha_Profile.objects.filter(User__is_active=1, Owner=username).exclude(Auth_Id=val)

        return render(request, 'Staff/index.html', {'update': True, 'user': user, 'prof': prof, 'val': val,
                                                    'exclude': exclude})
        ...
        users = BasicDetails.objects.filter(Asha_Worker=basic.Auth_Id)
...
def Add_Asha(request):
    if request.method == "POST":
        ...
        prof.save()
        account = Account(Auth_Id=prof, Pension='0', Salary='6000')
        account.save()
        return redirect('StaffHome')
    else:
        myDate = datetime.now() 
...
def SalaryDetails(request):
    sal = Account.objects.all()
    arr = []
    for data in sal:
        if data.Auth_Id.Owner == request.session['username']:
            arr.append(data)

    return render(request, 'Staff/SalaryDetails.html', {'sal': arr})
